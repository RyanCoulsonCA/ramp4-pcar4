import { $ as e, B as t, G as n, V as r, ct as i, s as a } from "./main-y6jdjIHV.js";
...
var Re = class extends a {
	...
	let u = null, d = 0, f = 0, p = [], m = null, ee = null, g = null, _ = null, v = null, y = null, b = /* @__PURE__ */ new Map(), x = /* @__PURE__ */ new Map(), ne = [], S = [], C = [], re = Te(""), ie = () => t.graphics.some((e) => {
		...
		return (t.measurementsEnabled || ie()) && (ut().forEach((e) => X(r, e)), dt().forEach((e) => X(r, e)), X(r, ft(e, n))), X(r, pt()), Array.from(r.values()).flatMap((e) => st(e));
	}, ht = () => {
		...
		d++, u !== null && (window.cancelAnimationFrame(u), u = null), m = null, re.value = "", gt(), J(), Qe();
		try {
	...
	setup(n) {
		let { t: a, availableLocales: l, locale: u } = ke(), f = (e) => a(e ? `draw.${e}` : "draw.unknown"), g = ye("iApi"), b = E(), C = t(), w = (e) => {
		...
		}, st = () => y(b.styleSettings), ct = () => re(b.bufferSettings), lt = () => b.identifyBufferMode, X = (e) => e ? le(e) : void 0, ut = (e) => !!e.attributes?.id && !e.attributes?.drawMeasurement && !e.attributes?.drawBufferFor, dt = async (e) => {
			...
			let t = k?.graphics.toArray().some((t) => X(t) === e) ?? !1, n = b.graphics.some((t) => X(t) === e);
			return !t && !n;
		}, _t = (e) => {
			switch (e) {
				case "point":
		...
		}, bt = (e) => oe(e.attributes), xt = (e) => ne(e.attributes), St = (e) => _(e.attributes), Ct = (e) => ae(e.attributes), wt = () => {
			if (!O) return;
			let e = st();
			...
			if (!k) return;
			let e = k.graphics.toArray().filter(ut);
			if (e.length) try {
				k.removeMany(e);
			} catch (e) {
			...
			if (!t || !n) return;
			let r = xt(n), i = St(n), a = oe(t.attributes), o = ne(t.attributes), s = _(t.attributes), c = r.distance !== o.distance || r.unit !== o.unit || i !== s;
			kt(n, a, o, s), B?.sourceGraphic === n && kt(B.editGraphic, a, o, s), Pt(n), Tt(n), Q(Z() ? n : void 0), c && Kt(n);
		}, It = (e, t = !0) => {
			let n = Dt(e);
			n && (R = n, Tt(n), n.attributes?.id && b.selectGraphic(n.attributes.id), Q(t ? n : void 0));
		}, Lt = (e) => {
			It(e), $t("details"), b.requestShapePanelFocus(), Kt(e);
		}, Rt = (e) => {
			It(e, !1), Wt(e);
		}, zt = (e) => {
			if (e) return e;
			...
			return B?.sourceGraphic === e && $({ restoreSource: !1 }), jt(t), Jt(t), qt(), k?.remove(e), b.removeGraphic(t), R = null, b.clearSelection(), Q(void 0), W(), g.updateAlert(a("draw.graphic.deleted")), !0;
		}, Vt = async (e) => {
			...
			for (let t of e) try {
				...
				let r = await ft(e, t.id), i = t.type || r.type, a = new h({
					...
					attributes: {
						type: i,
						drawStyle: y(t.settings.drawStyle),
						drawBuffer: re(t.settings.drawBuffer),
						drawIdentifyBufferMode: t.settings.drawIdentifyBufferMode,
						drawMapLabels: T(t.settings.drawMapLabels)
					}
				}), o = At(a, i, t.id);
				n.push(a), b.addGraphic({
					id: o,
					type: i,
					geometry: a.geometry,
			...
			return n.length ? t === k ? (t.addMany(n), n.forEach(Pt), W(), g.updateAlert(a("draw.import.success", { count: n.length })), n.length) : 0 : (g.updateAlert(a("draw.import.error.invalid")), 0);
		}, Ht = async () => {
			let e = b.importShapesRequestId;
			!e || e === Ie || !k || (await Vt([...b.importShapeRecords]), Ie = e, b.clearImportShapes(e));
		}, Ut = async (e) => {
			let t = k;
			if (!t || !b.graphics.length) return;
			let n = new Set(t.graphics.toArray().map((e) => X(e)).filter((e) => !!e)), r = [];
			for (let e of b.graphics) {
			...
			r.length && (e !== void 0 && e !== F || t !== k || (t.addMany(r), r.forEach(Pt)));
		}, Wt = (e) => {
			let t = ++Ae;
			b.setActiveTool("edit"), $t("edit"), window.setTimeout(() => {
			...
			getSelectedFeatureCountGraphic: () => Gt() ?? b.getSelectedGraphic() ?? void 0
		}), Zt = (e) => ce(C, e), Z = () => Zt(v), Qt = () => Z() && b.shapeDetailsActiveTab === "details" && !!b.selectedGraphicId, $t = (e = "details") => {
			...
			let t = V, n = new Set(t.minimizedPanels.map(({ panel: e }) => e.id)), r = un().filter(({ panel: e }) => !n.has(e.id));
			t.minimizedPanels.push(...r), pn(t, ++Fe, e);
		}, gn = () => {
			let e = V;
			if (!e) return;
			V = null, Fe++, e.paddingAdjusted && on(e.previousPadding), dn(e.minimizedPanels);
			let t = g.geo.map.esriView;
			...
			return k.graphics.toArray().slice().reverse().find((e) => !!e.attributes?.id && !e.attributes?.drawMeasurement && !e.attributes?.drawBufferFor && yn(e, t, r));
		}, Sn = (e) => {
			let t = Z();
			if (!_n() && !t) return;
			let n = xn(e);
			...
			U = e.register("D", {
				...
				keys: [
					...n,
					{
						...
						handler: () => (b.setActiveTool(null), $t("details"), "reset")
					},
					{
						key: "D",
						description: w("draw.keyboard.key.defaults"),
						handler: () => (b.setActiveTool(null), en(), "reset")
					}
				]
			});
		}
		let Q = (e) => {
			...
			if (!e) return;
			let t = b.graphics.find((t) => t.id === e), n = Et(e);
			!t || !n || (n.attributes = {
				...n.attributes ?? {},
				drawMapLabels: T(Ct(t))
			});
		}, Dn = () => {
			if (wn(), !Qt()) return;
			let e = de(b.activeSegmentKey);
			if (!e || e.kind !== "segment" || e.graphicId !== b.selectedGraphicId) return;
			let t = Et(e.graphicId), n = me(t?.geometry, e.graphicId)[e.index];
		...
		}, jn = (e) => B?.editGraphic === e ? B.sourceGraphic : e, Mn = () => {
			if (!B) return;
			...
			if (Ot(t), A.graphics = [t], e.visible = !1, B = {
				sourceGraphic: e,
				editGraphic: t,
				originalGeometry: n
			}, !Nn(t)) {
			...
			let i = (e) => {
				Te = window.setTimeout(() => {
					Te = null, !(!O || b.activeTool !== "edit" || R !== n) && In(r).then((t) => {
						if (!t) {
							if (e <= 0) {
								$({ restoreSource: !0 });
								return;
			...
			if (Ae++, J(), Mn(), q(), $({ restoreSource: !0 }), e && (R = null, b.clearSelection()), b.activeTool === "edit") {
				b.setActiveTool(null);
				return;
			}
			Q(Z() ? Gt() : void 0);
			...
			Ae++, J();
			let t = B;
			t?.sourceGraphic && t.originalGeometry && (t.sourceGraphic.geometry = t.originalGeometry, t.sourceGraphic.set("geometry", t.originalGeometry), Ot(t.sourceGraphic), Pt(t.sourceGraphic), Tt(t.sourceGraphic), qt(), Kt(t.sourceGraphic)), q(), $({ restoreSource: !0 }), e && (R = null, b.clearSelection()), b.activeTool === "edit" && b.setActiveTool(null), Q(Z() ? Gt() : void 0), W();
		};
		L(() => b.selectedGraphicId, (e, t) => {
			if (!(!O || !k)) {
				if (!e) J(), q(), $({ restoreSource: !0 }), Q();
					...
					t && (It(t, b.activeTool !== "edit"), b.shapeDetailsPickEnabled && Kt(t));
				}
				W();
			}
			...
			Rn({ clearSelection: b.stopEditModeClearSelection });
		}), L(() => b.cancelEditModeRequestId, () => {
			zn({ clearSelection: b.cancelEditModeClearSelection });
			...
			if (Qe(), e && !t) {
				b.activeTool !== "edit" && Q(Gt()), W(), mn();
				return;
			}
			!e && t && (b.activeTool === "edit" && Rn(), b.setShapeDetailsPickEnabled(!1), b.setShapeDetailsLabelsVisible(!1), b.setShapeDetailsLabelsUseSettings(!1), b.setShapeDetailsActiveTab("details"), b.clearMeasurementInteraction(), Q(void 0), W(), gn());
		...
		let { handleNavigationKeyDown: Bn, handleGraphicKeyboardEdit: Vn, resetMultiPointState: Hn } = He({
			iApi: g,
			drawStore: b,
			t: a,
			translateTerm: f,
			...
			let t = b.activeTool === "edit", n = vn();
			if (!t && !n) return;
			let r = Dt((await g.geo.map.esriView.hitTest(e, { include: k })).results.find((e) => "graphic" in e && e.graphic.layer === k && !!Dt(e.graphic))?.graphic) ?? bn(e);
			if (r) {
				if (n) {
					Rt(r);
					return;
			...
			let t = await dt(je);
			if (e !== F || !g.geo.map.esriView || (k = t?.esriLayer, !k) || (await mt(e), e !== F || !g.geo.map.esriView)) return;
			let n = await dt(Me);
			...
			}), tt(!1, i), g.geo.map.esriView.ui.add(i, "bottom-right"), P ||= (i.addEventListener("arcgisCreate", $e), i.addEventListener("arcgisUpdate", K), !0), O = i, tt(!1), wt(), N = g.geo.map.esriView.on("click", Un), document.addEventListener("keydown", Bn), document.addEventListener("keydown", Vn, { capture: !0 }), b.activeTool && b.activeTool !== "edit" && O.create(b.activeTool), await Ut(e), !(e !== F || !g.geo.map.esriView) && (W(), Ht());
		}, Gn = () => {
			if (!U) return;
			let e = g.keyboardNav;
			if (!e) {
			...
			F++, e && Gn(), N &&= (N.remove(), null), $({ restoreSource: !0 }), A &&= (r ? (g.geo.map.esriView?.map?.remove(A), A.destroy()) : A.removeAll(), null), O && (g.geo.map.esriView && g.geo.map.esriView.ui.remove(O), J(), q(), i && O.destroy()), P && ue.value && (ue.value.removeEventListener("arcgisCreate", $e), ue.value.removeEventListener("arcgisUpdate", K), P = !1), document.removeEventListener("keydown", Bn), document.removeEventListener("keydown", Vn, { capture: !0 }), R = null, Q(void 0), j && (r ? (g.geo.map.esriView?.map?.remove(j), j.destroy()) : j.removeAll(), j = null, Le = null), wn(), Tn(), J(), qt(), Ae++, We(), Mt(), n && Nt(), b.clearSelection(), t && b.activeTool && b.setActiveTool(null), Hn(), O = null, k = null;
		}, qn = (e) => {
			...
			if (e.state === "start") {
				...
				J(), It(n, Z()), n.attributes?.id && g.updateAlert(a("draw.graphic.selected", { type: f(n.attributes?.type) }));
			} else if (e.state === "active") {
				Ot(t);
				let e = Mn() ?? n;
				Pt(e), Tt(e), Q(Z() ? e : void 0), Ge(e, e.attributes?.type), Yt(e);
			} else if (e.state === "complete") {
				let e = Mn() ?? n;
				$({ restoreSource: !0 }), e.attributes?.id && (Ot(e), Pt(e), Tt(e), qt(), Kt(e), g.updateAlert(a("draw.graphic.updated"))), Q(Z() ? e : void 0), W(e, e.attributes?.type);
			}
		};
		return xe(() => {
			Cn(), ie(), Wn(), H.push(g.event.on(r.MAP_DESTROYED, () => {
				Kn();
			})), H.push(g.event.on(r.MAP_CLICK, Sn)), H.push(g.event.on(r.MAP_REFRESH_START, () => {
				Kn({
					cleanupKeyboard: !1,
					clearActiveTool: !0,
			...
			if (Qe(e), O && (e !== "edit" && Ae++, J(), q(), e !== "edit" && $({ restoreSource: !0 }), Q(Z() ? Gt() : void 0), Hn(), W(), e !== "edit" && e)) try {
				O.create(e);
			} catch (e) {
			...
			gn(), G !== null && (g.fixture.get("panguard")?.setEnabled(G), G = null), Kn(), H.forEach((e) => g.event.off(e));
...
Chaque forme sélectionnée peut avoir des options différentes des valeurs par défaut. Utilisez l'onglet Style de l'inspecteur de forme pour les couleurs et l'opacité. Utilisez l'onglet Modifier de l'inspecteur de forme pour la distance du tampon, l'unité du tampon, l'utilisation du tampon d'identification, les aides de modification et la suppression.`
	...
	async init() {
		...
		this.$iApi.panel.get("draw-settings") || (e[S] = {
			screens: { "draw-settings-screen": () => F(import("./draw-defaults-screen-D2YWjjsB.js")) },
			style: { width: "350px" },
			button: {
			...
			screens: { "draw-shape-details-screen": () => F(import("./shape-inspector-screen-7Ib3s40e.js")) },
			style: { width: "350px" },
			button: {
