...
import "tiny-emitter";
import { a as fe, _ as ge, G as Q, O as Ft, Q as Tt, L as $t, V as Rt, W as tt, X as re, i as lt, Y as Ye, Z as Le, $ as St } from "./main-d9txDjBC.js";
...
const Zt = { class: "h-full flex items-center justify-center" }, Qt = ["placeholder", "aria-label", "disabled"], Xt = ["placeholder", "aria-label", "disabled"], Jt = {
  ...
  props: ["params"],
  setup(_) {
    const n = C("none"), e = _, a = oe("iApi"), l = Tt(), s = C(), { t: u } = N(), p = O(() => {
  ...
  props: ["params"],
  setup(_) {
    const n = fe(), e = oe("iApi"), { t: a } = N(), l = C(), s = C(), u = C(!1), p = _, m = O(() => n.mobileView), b = () => {
      s.value?.textContent && (u.value = !0, l.value?._tippy.show(), navigator.clipboard.writeText(s.value?.textContent), setTimeout(() => {
        u.value = !1;
      }, 2e3));
    }, w = O(() => p.params.type === "number" ? p.params.value == null ? "" : e.ui.formatNumber(p.params.value) : p.params.type === "date" ? p.params.value == null ? "" : new Date(p.params.value).toISOString().slice(0, 10) : p.params.type === "string" ? !p.params.value || /<a[^>]*>[^<]+<\/a>/g.test(p.params.value) ? p.params.value : Dt(p.params.value, {
      target: "_blank",
      validate: {
        url: (L) => /^https?:\/\//.test(L)
        // only links that begin with a protocol will be hyperlinked
      }
    }) : ""), y = O(() => /<a[^>]*>[^<]+<\/a>/g.test(p.params.value) || /(http(s)?:\/\/.*)/g.test(p.params.value));
    return X(() => {
  ...
  setup(_) {
    const n = [re.OID, re.DOUBLE, re.SINGLE, re.INTEGER], e = oe("iApi"), a = lt(), l = fe(), s = O(() => l.mobileView), u = C(!s.value), p = C(), m = Mt("gridContainer"), { t: b, locale: w } = N(), y = () => At()?.proxy?.$forceUpdate(), L = _, c = C({
      id: "dummy",
      layerIds: [],
      state: new Gt(),
      fieldMap: {}
